...
export function SustratoLogoRotating({
  ...
  const gradientIdRef = useRef(
    `logoGradient_${Math.random().toString(36).substr(2, 9)}`
  );
  const maskId1Ref = useRef(
    `path-2-outside-1-${Math.random().toString(36).substr(2, 9)}`
  );
  const maskId2Ref = useRef(
    `path-4-outside-2-${Math.random().toString(36).substr(2, 9)}`
  );
