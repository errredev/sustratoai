...
export function SustratoLoadingLogo({
  ...
  // Obtener los colores de acento para la transición
  const accentColors = {
    bg: component?.icon?.colors?.accent?.bg || "#e0b7ff",
    pure: component?.icon?.colors?.accent?.pure || "#9333ea",
    text: component?.icon?.colors?.accent?.text || "#6b21a8",
    dark: component?.icon?.colors?.accent?.dark || "#581c87",
  }
  ...
  // Obtener el color actual del círculo interior
  const innerCircleColor = colorTransition
    ? getCurrentAccentColor()
    : accentColor || component?.icon?.colors?.accent?.pure || "currentColor"
    ...
    <div className="flex flex-col items-center justify-center">
      ...
      {variant === "dash" || variant === "progress" ? (
        <div className={cn("relative", className)}>
          ...
          >
            <circle
              ...
              cy="50"
              r="45"
              stroke={primaryColor || component?.icon?.colors?.primary?.pure || "currentColor"}
              strokeWidth="6"
              fill="none"
            />
          </svg>
        ...
        <div className={cn(animationClasses[variant], className)}>
          ...
          <div className="relative">
            <SustratoLogo
              size={size}
              primaryColor={primaryColor || component?.icon?.colors?.primary?.pure}
              secondaryColor={secondaryColor || component?.icon?.colors?.secondary?.pure}
              // No pasamos accentColor aquí porque lo reemplazaremos
      ...
      {showText && (
        <p
          ...
          style={{ color: primaryColor || component?.icon?.colors?.primary?.text }}
        >
          {text}
        </p>
      )}
