...
import { Text } from "@/components/ui/text"; // Importar Text por si se usa en el mensaje de error
...
export default function VerMiembroPage() {
  ...
  const cargarDatosCompletos = useCallback(async () => {
    ...
    try {
      ...
      if (isMounted) {
        setError(
          `Error inesperado al cargar datos: ${(err as Error).message}`
        );
      }
    } finally {
      if (isMounted) {
    ...
    return () => { isMounted = false; }; // Cleanup function para el useCallback
  ...
  }, [proyectoActual?.id, memberId, cargarDatosCompletos]); // QUITAR isLoading de las dependencias
  ...
  const getNombreMiembro = (): string => {
    ...
    return miembro.profile?.public_contact_email || `Usuario ID: ${miembro.user_id.substring(0,8)}...`;
  };
  ...
  const valoresFormulario: MiembroFormValues | undefined = miembro
    ? {
        emailUsuario:
          ...
          (miembro.user_id ? `No disponible (ID: ${miembro.user_id.substring(0,8)}...)` : "Email no disponible"),
     ...
     return (
      <div className="space-y-6">
        <PageHeader
          ...
          description={error || "No se pudo cargar la información del miembro o el miembro no existe."} // Mostrar error si existe
          actions={
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Información del miembro en el proyecto (solo lectura)."
        actions={
      ...
      {error && miembro && ( // Si hay un error (ej. roles) pero el miembro SÍ cargó
        <div className="mt-4 p-4 bg-warning-muted text-warning-foreground rounded-md text-center"> {/* Cambiado a warning */}
          <Text variant="caption" weight="medium">Advertencia:</Text>
          <Text variant="caption" className="mt-1">{error}</Text>
          <Text variant="caption" size="xs" className="mt-1 opacity-80">
            (Se muestran los datos de miembro disponibles. Alguna información adicional, como la lista completa de roles, podría no haberse cargado correctamente).
          </Text>
        </div>
      )}
    </div>
